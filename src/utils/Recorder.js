 ...
 * @Desc    :   None
 */
const Recorder = function (onaudioprocess, showWaveFn) {
  this.config = {
...
Recorder.prototype.SRC = function (input, inputFs, outputFs) {
  ...
  // 输出音频长度
  const outlen = Math.round((len * outputFs) / inputFs);
...
Recorder.prototype.compress = function (isReplay) {
  ...
  const result = this.SRC(data, this.inputSampleRate, this.outputSampleRate);
  return result;
};
/**
 * 转换到我们需要的对应格式的编码
...
Recorder.prototype.encodePCM = function (isReplay) {
  const bytes = this.compress(isReplay);
...
Recorder.prototype.encodePCMFragment = function (fragment) {
  const data = new Float32Array(fragment);
  ...
  const bytes = this.SRC(data, this.inputSampleRate, this.outputSampleRate);
  const sampleBits = Math.min(this.inputSampleBits, this.oututSampleBits);
  let offset = 0;
  const dataLength = bytes.length * (sampleBits / 8);
  const buffer = new ArrayBuffer(dataLength);
  const lastData = new DataView(buffer);
...
function combineDataView(ResultConstructor, ...arrays) {
  ...
  const result = new ResultConstructor(buffer);
  // 设置数据
  arrays.forEach((arr) => {
    // dataview合并
...
Recorder.prototype.encodeWAV = function (isReplay) {
  ...
  const bytes = this.encodePCM(isReplay);
...
Recorder.prototype.toWaveData = function (o) {
  const PowerLevel = function (pcmAbsSum, pcmLength) {
  ...
  const sizeOld = this.recSize;
  const addSize = pcm.length;

  const bufferSize = sizeOld + addSize;
  this.recSize = bufferSize;

  const bufferSampleRate = this.inputSampleRate;
  const powerLevel = PowerLevel(sum, pcm.length);
  const duration = Math.round((bufferSize / bufferSampleRate) * 1000);
